      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="text-center sm:text-center">
          ...
          <DialogTitle className="text-xl font-semibold">
            {tr('onboarding.welcome.title', 'Benvingut a Summa Social')}
          </DialogTitle>
          <DialogDescription className="text-center space-y-3 pt-2">
            <p>
              {tr('onboarding.welcome.body1', 'Et guiarem pels passos bàsics per configurar la teva organització: dades fiscals, firma per certificats i categories de moviments.')}
            </p>
            <p>
              {tr('onboarding.welcome.body2', 'Pots fer-ho ara o més endavant des de Configuració.')}
            </p>
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-3 pt-4">
          <Button onClick={handleStartGuide} className="w-full">
            {tr('onboarding.welcome.ctaGuide', "Guia'm")}
            <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
          <Button variant="outline" onClick={handleSkip} className="w-full">
            {tr('onboarding.welcome.ctaSkip', 'Començar pel meu compte')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
