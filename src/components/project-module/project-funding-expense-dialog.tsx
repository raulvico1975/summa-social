...
import { getFundingExpenseStatus } from '@/lib/project-module-funding';
import type {
  BudgetLine,
  ProjectFundingAllocationKind,
...
function getImputedAmount(expense: UnifiedExpenseWithLink): number {
...
export function ProjectFundingExpenseDialog({
  ...
  const activeSources = React.useMemo(
    () => fundingSources.filter((source) => source.archivedAt === null),
    [fundingSources]
  );
  const [lines, setLines] = React.useState<ProjectFundingExpenseAllocationFormLine[]>([]);
  const imputedAmount = expense ? getImputedAmount(expense) : 0;
  const distributedAmount = lines.reduce((sum, line) => {
    const parsed = Number.parseFloat(line.amountEUR.replace(',', '.'));
  ...
  React.useEffect(() => {
    ...
    const existing = allocations
      ...
      .map((allocation) => ({
        ...
        amountEUR: String(allocation.amountEUR),
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {lines.map((line, index) => (
              <div key={index} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-[1.2fr_120px_120px_1fr_auto]">
                ...
                <div className="space-y-1">
                  ...
                  <Input inputMode="decimal" value={line.amountEUR} onChange={(event) => updateLine(index, { amountEUR: event.target.value })} />
                </div>
                <div className="space-y-1">
                  <Label>{tr('projectModule.multiFunding.kind')}</Label>
                  <Select value={line.kind} onValueChange={(value) => updateLine(index, { kind: value as ProjectFundingAllocationKind })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="cash">{tr('projectModule.multiFunding.kindCash')}</SelectItem>
                      <SelectItem value="in_kind">{tr('projectModule.multiFunding.kindInKind')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                ...
                <div className="flex items-end gap-1">
                  <Button type="button" variant="outline" size="sm" onClick={() => updateLine(index, { amountEUR: pendingAmount.toFixed(2) })}>
