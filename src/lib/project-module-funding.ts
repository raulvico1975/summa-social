...
export type ProjectFundingImputedAmountResolver = (input: {
  link: ExpenseLink;
...
export interface ProjectFundingImputedAmountOptions {
  expense?: UnifiedExpense | null;
...
export function getProjectImputedAmountForExpense(
  link: ExpenseLink,
  projectId: string,
  options: ProjectFundingImputedAmountOptions = {}
): number {
  const { expense = null, resolveAssignmentAmountEUR } = options;
  return roundCurrency(
    link.assignments
      .filter((assignment) => assignment.projectId === projectId)
      .reduce((sum, assignment) => {
        const amountEUR = resolveAssignmentAmountEUR
          ? resolveAssignmentAmountEUR({ link, assignment, expense, projectId })
          : assignment.amountEUR;
        return sum + (amountEUR != null ? Math.abs(amountEUR) : 0);
      }, 0)
  );
}
