...
describe('generateModel182AEATFile', () => {
  ...
  it('donant amb múltiples errors → tots els motius a excluded', () => {
    ...
    assert.strictEqual(result.excludedCount, 1);
    // L'exclusió ha de contenir els 3 issueCodes
    const exc = result.excluded[0];
    assert.strictEqual(exc.name, 'Donant Molt Invàlid');
    assert.ok(exc.issueCodes.includes('TAXID_EMPTY'));
    assert.ok(exc.issueCodes.includes('ZIPCODE_INCOMPLETE'));
    assert.ok(exc.issueCodes.includes('DONOR_TYPE_MISSING'));
  });
