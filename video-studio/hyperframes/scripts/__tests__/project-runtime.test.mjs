...
test('createRuntimeProject builds a temporary render root without mutating the repo', async () => {
  const runtimeRoot = await createRuntimeProject(projectRoot, '05-devolucions-estat-real-16x9');

  const indexHtml = await fs.readFile(path.join(runtimeRoot, 'index.html'), 'utf8');

  assert.match(indexHtml, /data-composition-id="05-devolucions-estat-real-16x9"/);
  assert.match(indexHtml, /data-composition-src="compositions\/05-devolucions-estat-real-16x9\.html"/);
