...
## 6. SuperAdmin Provider — ✅ FIX APLICAT

**Escenari:** SuperAdmin rep intermitentment "No tens accés a aquesta organització".

**Causa:**
- UID hardcoded (`SUPER_ADMIN_UID`) en lloc de consultar `systemSuperAdmins`
- Errors de permisos transitoris sense retry

**Solució aplicada:**

1. **Check dinàmic de SuperAdmin** via col·lecció `systemSuperAdmins`:
```typescript
...
2. **Retry únic** en cas d'error de permisos:
```typescript
if (isPermissionDenied(err) && !superAdminRetryRef.current && user) {
  superAdminRetryRef.current = true;
  // Check SuperAdmin i retry si escau
}
```
...
**Guardrails:**
- ✅ Màxim 1 retry per sessió (via `useRef`)
- ✅ Source of truth: `systemSuperAdmins` collection
- ✅ No hardcoded UIDs
...
| 2026-01-04 | SuperAdmin provider: systemSuperAdmins + retry | ✅ Desplegat |
