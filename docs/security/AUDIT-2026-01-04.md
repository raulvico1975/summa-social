...
| Imports idempotència | **PARCIAL** | **P1 alt** (segmentació 500 pendent) |
| Exports | **PASS** | Baix |
...
### 4.2 Imports Idempotència — ✅ PARCIALMENT IMPLEMENTAT
...
| Segmentació 500 | ⚠️ | **NO IMPLEMENTAT** - Un sol batch sense límit |

**Risc residual:** Si s'importen >500 transaccions, Firestore fallarà amb error de quota.
...
**Prioritat:** P1 alt (pot causar fallida en imports grans)
...
## 7d. AuthRedirectGuard — Redirect immediat a logout (P0 UX)

**Problema:** Després del fix anterior (ignorar errors amb `auth: null`), l'usuari es quedava a `/dashboard` amb dades buides en lloc de ser redirigit a login.

**Causa:** L'`OrganizationProvider` ja tenia un redirect quan `!user`, però:
1. El redirect podia no executar-se prou ràpid
2. Altres components fora del provider podien quedar-se sense redirect

**Solució:** Component `AuthRedirectGuard` al `FirebaseProvider` que detecta la transició de logged in a logged out i fa redirect immediat.
...
export function AuthRedirectGuard() {
  ...
  useEffect(() => {
    ...
    if (wasLoggedIn === true && !isLoggedIn) {
      // Si ja estem a login o path públic, no redirigir
...
**Guardrails:**
- ✅ Redirect immediat quan l'usuari fa logout
