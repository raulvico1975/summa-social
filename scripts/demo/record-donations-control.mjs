...
const DEFAULT_TMP_DIR = path.join(process.cwd(), 'tmp', 'donations-control-demo');
const DEFAULT_EMAIL = 'demo.recorder@summasocial.local';
const DEFAULT_PASSWORD = 'DemoRecorder!2026';
const SCENARIO_SLUG = 'donations-control-demo';
const DEFAULT_OUTPUT_DIR = path.join(process.cwd(), 'output', 'playwright', SCENARIO_SLUG);
const DEMO_DONOR_ID = 'demo_control_donor_001';
const DEMO_DONOR_NAME = 'Clara Soler Ribas';
...
async function ensureDemoRecorder(auth, db, email, password) {
  ...
  await Promise.all([
    db.doc(`systemSuperAdmins/${uid}`).set({
      email: normalizedEmail,
      createdAt: nowIso,
      createdBy: 'record-donations-control-script',
      autoCreated: true,
    }, { merge: true }),
    ...
    db.doc(`organizations/${DEMO_ORG_ID}/members/${uid}`).set({
      ...
      role: 'admin',
      joinedAt: nowIso,
      invitationId: 'record-donations-control-script',
    }, { merge: true }),
  ]);
...
async function assertDemoDonorExists(db) {
  const donorDoc = await db.doc(`organizations/${DEMO_ORG_ID}/contacts/${DEMO_DONOR_ID}`).get();
  if (!donorDoc.exists) {
...
async function openDonorsPage(page, credentials, artifactDir) {
  ...
  await page.screenshot({
    path: path.join(artifactDir, 'donations-control-start.png'),
    fullPage: false,
  });
}
...
async function openDonorDetail(page, donorName) {
  const donorButton = page.getByRole('button', { name: new RegExp(donorName, 'i') }).first();
  await donorButton.waitFor({ state: 'visible', timeout: 30000 });
  await moveAndClick(page, donorButton);
  await page.getByText(/RESUM|RESUMEN/i).waitFor({ state: 'visible', timeout: 30000 });
  await page.getByText(/Import net certificable|Importe neto certificable/i).waitFor({
    state: 'visible',
    timeout: 30000,
  });
  await sleep(2000);
...
async function runFlow(page, artifactDir) {
  ...
  await typeSearch(page, searchInput, DEMO_DONOR_NAME);
  await page.screenshot({
    path: path.join(artifactDir, 'donations-control-search.png'),
    fullPage: false,
  });

  await openDonorDetail(page, DEMO_DONOR_NAME);
  await sleep(2600);

  await page.screenshot({
    path: path.join(artifactDir, 'donations-control-detail.png'),
    fullPage: false,
  });
}
...
async function main() {
  ...
  await assertDemoDonorExists(db);
  ...
  try {
    await openDonorsPage(page, credentials, OUTPUT_DIR);
